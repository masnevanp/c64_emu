...
bool write_prg_file(const std::string& filename, u16 start_addr, u8* data, size_t sz) {
...
void Tape::Virtual::load(System::CPU& cpu, u8* ram) {
    ...
    auto bin = read_bin_file(filepath);
    auto sz = bin.size();
    if (sz > 2) {
...
void Tape::Virtual::save(System::CPU& cpu, u8* ram) {
    ...
    write_prg_file(filepath, start_addr, &ram[start_addr], sz);
